...
/*     */ {
/*  22 */   private static final char[] HEX_CHARS = ;
/*     */   
...
/*     */   {
/*  80 */     SoftReference<JsonStringEncoder> ref = (SoftReference)_threadEncoder.get();
...
/* 108 */     int[] escCodes = CharTypes.get7BitOutputEscapes();
...
/* 141 */         System.arraycopy(this._quoteBuffer, first, outputBuffer, 0, second);
/* 142 */         outPtr = second;
/*     */       } else {
...
/*     */     }
/*     */     label239:
/* 149 */     textBuffer.setCurrentLength(outPtr);
...
/* 171 */       int[] escCodes = CharTypes.get7BitOutputEscapes();
/*     */       
/*     */       for (;;)
/*     */       {
...
/*     */         
/* 197 */         outputPtr = _appendByteEscape(ch, escape, byteBuilder, outputPtr);
...
/*     */     label496:
/* 245 */     return this._byteBuilder.completeAndCoalesce(outputPtr);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */     label447:
/* 335 */     return this._byteBuilder.completeAndCoalesce(outputPtr);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/* 362 */     if (escCode < 0) {
/* 363 */       byteBuilder.append(117);
/* 364 */       if (ch > 255) {
/* 365 */         int hi = ch >> 8;
/* 366 */         byteBuilder.append(HEX_BYTES[(hi >> 4)]);
/* 367 */         byteBuilder.append(HEX_BYTES[(hi & 0xF)]);
/* 368 */         ch &= 0xFF;
/*     */       } else {
/* 370 */         byteBuilder.append(48);
/* 371 */         byteBuilder.append(48);
/*     */       }
/* 373 */       byteBuilder.append(HEX_BYTES[(ch >> 4)]);
/* 374 */       byteBuilder.append(HEX_BYTES[(ch & 0xF)]);
/*     */     } else {
/* 376 */       byteBuilder.append((byte)escCode);
/*     */     }
/* 378 */     return byteBuilder.getCurrentSegmentLength();
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */   {
/* 387 */     if ((secondPart < 56320) || (secondPart > 57343)) {
/* 388 */       throw new IllegalArgumentException("Broken surrogate pair: first char 0x" + Integer.toHexString(firstPart) + ", second 0x" + Integer.toHexString(secondPart) + "; illegal combination");
/*     */     }
/* 390 */     return 65536 + (firstPart - 55296 << 10) + (secondPart - 56320);
/*     */   }
/*     */   
/*     */   private void _throwIllegalSurrogate(int code)
/*     */   {
/* 395 */     if (code > 1114111) {
/* 396 */       throw new IllegalArgumentException("Illegal character point (0x" + Integer.toHexString(code) + ") to output; max is 0x10FFFF as per RFC 4627");
/*     */     }
/* 398 */     if (code >= 55296) {
/* 399 */       if (code <= 56319) {
/* 400 */         throw new IllegalArgumentException("Unmatched first part of surrogate pair (0x" + Integer.toHexString(code) + ")");
/*     */       }
/* 402 */       throw new IllegalArgumentException("Unmatched second part of surrogate pair (0x" + Integer.toHexString(code) + ")");
/*     */     }
/*     */     
/* 405 */     throw new IllegalArgumentException("Illegal character point (0x" + Integer.toHexString(code) + ") to output");
/*     */   }
/*     */ }


/* Location:              D:\EclipseWorkspace\jackson-core-asl-1.9.11.jar!\org\codehaus\jackson\io\JsonStringEncoder.class
